...
- (void)loadView
{
    ...
    [self.firstConversationCueView autoPinEdgeToSuperviewEdge:ALEdgeTrailing withInset:93.f];
...
- (void)createFirstConversationCueView
{
    ...
    shapeLayer.fillColor = Theme.orangeBackground.CGColor;
...
- (void)updateBarButtonItems
{
    ...
    //  Settings button.
    const NSUInteger kAvatarSize = 38;
    ...
    [avatarButton.imageView setContentMode:UIViewContentModeScaleAspectFit];
    ...
    UIView* wrapAvatarButton = [UIView new];
    wrapAvatarButton.backgroundColor = [UIColor clearColor];
    [wrapAvatarButton addSubview:avatarButton];
    [wrapAvatarButton autoSetDimension:ALDimensionWidth toSize:60];
    [wrapAvatarButton autoSetDimension:ALDimensionHeight toSize:60];
    
    [avatarButton autoHCenterInSuperview];
    [avatarButton autoVCenterInSuperview];
    
    UIBarButtonItem *settingsButton = [[UIBarButtonItem alloc] initWithCustomView:wrapAvatarButton];
    ...
    switch (self.conversationListMode) {
        case ConversationListMode_Inbox:
            leftTitle = NSLocalizedString(@"HOME_VIEW_TITLE_INBOX", @"Title for the conversation list's default mode.");
            break;
        case ConversationListMode_Archive:
            leftTitle = NSLocalizedString(@"HOME_VIEW_TITLE_ARCHIVE", @"Title for the conversation list's 'archive' mode.");
            break;
    }
    self.navigationItem.leftBarButtonItem = [[UIBarButtonItem alloc] initWithTitle: leftTitle style:UIBarButtonItemStylePlain target:self action: nil];
